...
from deepsynthesis.layers import attention
from deepsynthesis.layers import positional_encoding as pe
from deepsynthesis.models.beam_search import BeamSearch
...
def create_model(params, training, custom_pos_enc=False, attn_weights=False):
    ...
    if training:
        ...
        if attn_weights:
            outputs, scores, enc_attn_weights, dec_attn_weights = results[
                'outputs'], results['scores'], results[
                    'enc_attn_weights'], results['dec_attn_weights']
...
class TransformerMetricsLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.accuracy_per_sequence_mean = tf.keras.metrics.Mean(
    ...
    def call(self, inputs):
        ...
        weights = tf.cast(tf.not_equal(targets, self.params['target_pad_id']),
        ...
        incorrect_predictions = tf.cast(tf.not_equal(outputs, targets),
                                        self.dtype) * weights
        correct_sequences = 1.0 - tf.minimum(
            1.0, tf.reduce_sum(incorrect_predictions, axis=-1))
...
class TransformerEncoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, params):
        ...
        self.ff = tf.keras.Sequential([
            tf.keras.layers.Dense(params['d_ff'],
                                  activation=params['ff_activation']),
...
class TransformerDecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self,
        ...
        enc_dec_attn = self.dropout_enc_dec_attn(enc_dec_attn,
                                                 training=training)
        norm_enc_dec_attn = self.norm_enc_dec_attn(enc_dec_attn +
                                                   norm_self_attn)
...
class TransformerEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, params):
        ...
        self.enc_layers = [
            ...
            for _ in range(params['num_layers_enc'])
        ]
...
class TransformerDecoder(tf.keras.layers.Layer):
    ...
    def __init__(self, params):
        ...
        self.dec_layers = [
            ...
            for _ in range(params['num_layers_dec'])
        ]
...
class Transformer(tf.keras.Model):
    ...
    def __init__(self, params):
        ...
        Args:
            params: hyperparameter dictionary containing the following keys:
                ...
                ff_activation: string, activation function used in feed-forward networks
                num_heads: int, number of attention heads
                num_layers_enc: int, number of encoder layer
    ...
    def call(self, inputs, training):
        ...
        if 'target' in inputs:
            ...
            return self.decode(target, encoder_output, input_padding_mask,
                               training)
        else:
            return self.predict(encoder_output, encoder_attn_weights,
                                input_padding_mask, training)
    ...
    def encode(self, inputs, padding_mask, positional_encoding, training):
        ...
        encoder_output, attn_weights = self.encoder_stack(
            input_embedding, padding_mask, training)
    ...
    def predict(self, encoder_output, encoder_attn_weights, input_padding_mask,
        ...
        def logits_fn(ids, i, cache):
            ...
            decoder_output, attn_weights = self.decoder_stack(
                decoder_input, cache['encoder_output'], self_attention_mask,
                cache['input_padding_mask'], training, cache)

            logits = self.final_projection(decoder_output)
            logits = tf.squeeze(logits, axis=[1])
            return logits, cache

        initial_ids = tf.ones([batch_size],
                              dtype=tf.int32) * self.params['target_start_id']
        ...
        cache = {
            'layer_%d' % layer: {
                'keys':
                    tf.zeros([batch_size, 0, num_heads, d_heads],
                             dtype=self.params['dtype_float']),
                'values':
                    tf.zeros([batch_size, 0, num_heads, d_heads],
                             dtype=self.params['dtype_float'])
        ...
        #_, decoder_attn_weights = self.decode(top_decoded_ids, encoder_output,
        #                                      input_padding_mask, training)
