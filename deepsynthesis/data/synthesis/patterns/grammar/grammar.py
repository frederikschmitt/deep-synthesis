...
from deepsynthesis.data.synthesis.patterns.grammar.patterns import *
...
DEFAULT_PARAMS = {
    ...
    'input_within_n_steps': {
        'max_n': 2
    },
    'output_after_n_steps': {
        'max_n': 2
    },
    'output_for_n_steps': {
        'max_n': 2
    },
    'output_within_n_steps': {
        'max_n': 2
    },
    'cnf_mutual_exclusion': {
        'min_num_vars': 2,
        'max_num_vars': 3
    },
    'dnf_mutual_exclusion': {
        'min_num_vars': 3,
        'max_num_vars': 3
    },
    'factorized_mutual_exclusion': {
        'min_num_vars': 3,
        'max_num_vars': 3
    }
}
...
class Grammar():
    ...
    def response(self):
        obl_react = [
            ...
            for (in_obl, out_react
                ) in product(self.input_obligation, self.output_reactivity)
        ]
        react_obl = [
            Response(in_react, out_obl)
            for (in_react, out_obl
                ) in product(self.input_reactivity, self.output_obligation)
        ]
    ...
    def precedence(self):
        obl_react = [
            ...
            for (out_obl, in_react
                ) in product(self.output_obligation, self.input_reactivity)
        ]
        react_obl = [
            Precedence(out_react, in_obl)
            for (out_react, in_obl
                ) in product(self.output_reactivity, self.input_obligation)
        ]
        return obl_react + react_obl
    ...
    def mutual_exclusion(self):
        return list(
            chain(self.cnf_mutual_exclusion, self.dnf_mutual_exclusion,
    ...
    def cnf_mutual_exclusion(self):
        ...
        return [
            CNFMutualExclusion([self.atomic_output
    ...
    def dnf_mutual_exclusion(self):
        ...
        return [
            DNFMutualExclusion([self.atomic_output
                                for _ in range(num_vars)])
            for num_vars in range(min_num_vars, max_num_vars + 1)
        ]
    ...
    def factorized_mutual_exclusion(self):
        min_num_vars = self.params['factorized_mutual_exclusion'][
            'min_num_vars']
        max_num_vars = self.params['factorized_mutual_exclusion'][
            'max_num_vars']
        cnf = [
            FactorizedCNFMutualExclusion(
                [self.atomic_output
                 for _ in range(num_vars)])
            for num_vars in range(min_num_vars, max_num_vars + 1)
        ]
        dnf = [
            FactorizedDNFMutualExclusion(
                [self.atomic_output
                 for _ in range(num_vars)])
            for num_vars in range(min_num_vars, max_num_vars + 1)
        ]
    ...
    def input_reactivity(self):
        return list(
            chain(self.input_obligation, self.input_recurrence,
    ...
    def input_obligation(self):
        return list(
            chain(self.input_after_n_steps, self.input_for_n_steps,
                  self.input_within_n_steps, self.eventually_input,
                  self.globally_input, self.input))

    @property
    @lru_cache()
    def output_reactivity(self):
        return list(
            chain(self.output_obligation, self.output_recurrence,
                  self.output_persistence))

    @property
    @lru_cache()
    def output_obligation(self):
        return list(
            chain(self.output_after_n_steps, self.output_for_n_steps,
                  self.output_within_n_steps, self.eventually_output,
                  self.globally_output, self.output))
    ...
    def input_after_n_steps(self):
        ...
        patterns = [
            AfterNSteps(input, n + 1)
            for (input, n) in product(self.input, range(max_n))
        ]
        return patterns
    ...
    def output_after_n_steps(self):
        ...
        return [
            AfterNSteps(output, n + 1)
            for (output, n) in product(self.output, range(max_n))
        ]
...
def grammar_cardinality(num_inputs, num_outputs):
    cardinality = 0
    for pattern in Grammar().derive_all():
        cardinality += factorial(num_inputs, pattern.num_inputs) * \
            factorial(num_outputs, pattern.num_outputs)
    return cardinality
...
def add_parser_args(parser):
    parser.add_argument('--num-inputs',
                        type=int,
                        default=5,
                        help='number of input atomic propositions')
    parser.add_argument('--num-outputs',
                        type=int,
                        default=5,
                        help='number of output atomic propositions')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Calculating grammar cardinality')
