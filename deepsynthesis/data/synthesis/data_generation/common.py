...
from deepsynthesis.data.synthesis.dataset import sample_to_csv_row
...
def add_parser_args(parser):
    parser.add_argument('--all-aps',
                        action='store_true',
                        help=('circuits incorporate all AP contained in the set'
                              'of guarantees'))
    parser.add_argument('--batch-size',
                        type=int,
                        default=10,
                        help='size of batches provided to worker')
    parser.add_argument('--max-frac-ands',
                        type=float,
                        default=None,
                        metavar='max',
                        help=('maximal fraction of circuits with'
                              '0,1,2,... ANG gates'))
    ...
    parser.add_argument('--num-ands',
                        action=argparse_min_max(),
                        nargs='*',
                        default=(0, None),
                        metavar=('min', 'max'),
                        help='number of AND gates in AIGER circuit')
    parser.add_argument('--num-ands-plus-latches',
                        action=argparse_min_max(),
                        nargs='*',
                        default=(0, None),
                        metavar=('min', 'max'),
                        help=('number of AND gates plus latches in AIGER'
                              'circuit'))
    parser.add_argument('--num-guarantees',
                        action=argparse_min_max(),
                        nargs='*',
                        default=(1, None),
                        metavar=('min', 'max'),
                        help='number of guarantees')
    parser.add_argument('--num-latches',
                        action=argparse_min_max(),
                        nargs='*',
                        default=(0, None),
                        metavar=('min', 'max'),
                        help='number of latches in AIGER circuit')
    parser.add_argument('--num-vars',
                        action=argparse_min_max(),
                        nargs='*',
                        default=(0, None),
                        metavar=('min', 'max'),
    ...
    parser.add_argument('--storage-dir',
                        default=os.path.expanduser('~/deeplogic-storage'),
                        help=('directory to store DeepLogic data, models'
                              'and temporary files'))
...
def check_upper_bounds(sample, params, counters):
    ...
    num_vars, _, num_latches, _, num_ands = aiger.header_ints_from_str(
        sample['circuit'])
    ...
    if params['max_frac_ands'] is not None:
        if counters['ands'].get(
                num_ands, 0) >= params['max_frac_ands'] * params['num_samples']:
...
@ray.remote
def csv_dataset_writer(queue, folder_path, num_samples, train_frac, val_frac,
                       test_frac):
    filepaths = []
    files = []
    file_writers = []
    for split in ['train', 'val', 'test']:
        ...
        file_writer.writerow(
            ['guarantees', 'inputs', 'outputs', 'realizable', 'circuit'])
    ...
    while sum(file_counts) < num_samples:
        ...
        if file_counts[index] == file_target_counts[index]:
            files[index].close()
            logger.info('%d samples written to %s', file_counts[index],
                        filepaths[index])
            file_probs[index] = 0
            if sum(file_probs) > 0:
...
@ray.remote
def csv_file_writer(queue, filepath):
...
class ProgressActor:
    ...
    def update(self, key, delta=1):
        if key in self.progress:
            ...
            self.progress[key] = delta
            self.progress = {
                key: self.progress[key] for key in sorted(self.progress.keys())
            }
            self.event.set()
